class SelectProductVariants {
    constructor(productVariantNode) {
        ...
        this.boxWrapper = productVariantNode.querySelector("#boxWrapper");
        this.productVariant =
            productVariantNode.querySelectorAll(".product-variant");
        this.hiddenInputProductVariant = productVariantNode.querySelector(
            "#variantHiddenInput"
        );

        this.handleProductVariant = this.handleProductVariant.bind(this);
        this.boxWrapper?.addEventListener("click", this.handleProductVariant);
        this.checkFirstLoad();
    }
    ...
    checkFirstLoad() {
        ...
        if (hiddenInputValue.trim()) {
            this.selectedVariantValue = hiddenInputValue
                .split("|")
                .map((value) => Number(value));
            this.productVariant.forEach((variant) => {
                if (
                    this.selectedVariantValue.includes(
                        Number(variant.getAttribute("value"))
                    )
                ) {
