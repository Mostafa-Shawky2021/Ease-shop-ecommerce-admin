                ...
                <table class="table-data-layout table">
                    ...
                    <tbody>
                        @forelse($order->products as $product)
                            @php
                                $isImageExternal = preg_match('/https?/', $product->image);
                                $productImage = $isImageExternal ? $product->image : asset('storage/' . $product->image);
                                $productPrice = $product->price_discount ?? $product->price;
                                
                            @endphp
                            <tr>
                                <td class="image">
                                    <img width="30" height="30" src="{{ $productImage }}"
